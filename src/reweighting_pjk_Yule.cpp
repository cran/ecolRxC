...
List reweighting_pjk_Yule_cpp(
  ...
  while (dif > tol && cont < 10) {
     for (int jj : parties1) {
      for (int kk : parties2) {
        ...
        // Marginales del sub-cubo [2 x 2 x I]
        ...
        for (int i = 0; i < I; ++i) {
          ...
          cond = cond &&
            (suma_rows(0,i) >= 1.0) && (suma_rows(0,i) == 0.0) &&
            (suma_cols(0,i) >= 1.0) && (suma_cols(0,i) == 0.0);
          valid[i] = cond;
        }
        
        // Diagnóstico de valid
        int n_valid = 0;
        for (int i = 0; i < I; ++i) if (valid[i]) n_valid++;
        // Transformaciones y correlación ponderada
        ...
        double k_yule = ((1.0 - cor_e) != 0.0) ? (1.0 + cor_e) / (1.0 - cor_e) : 0.0;
        
        // no.0 y actualización Yule
        for (int i = 0; i < I; ++i) {
          bool no0 = valid[i] &&
            !std::isnan(pjk_array(jj, kk, i)) &&
            (pjk_array(jj, kk, i) != 0.0) &&
            !std::isnan(pjk_array(ref1_0, ref2_0, i)) &&
            (pjk_array(ref1_0, ref2_0, i) != 0.0);
          if (!no0) continue;
          
          pjk_array(jj, kk, i) = k_yule *
            (pjk_array(jj, ref2_0, i) * pjk_array(ref1_0, kk, i)) /
              pjk_array(ref1_0, ref2_0, i);
        }
        
      } // kk
    } // jj
    
    // Clamp: NaN, Inf y negativos a cero
    for (auto& v : pjk_array) {
      if (std::isnan(v) || std::isinf(v) || v < 0.0) v = 0.0;
    }
    ...
    // Reescalado columnas de referencia
    for (int kk : parties2) {
      for (int i = 0; i < I; ++i) {
        if (!std::isnan(p_est_cols(i, kk)) && p_est_cols(i, kk) != 0.0) {
          pjk_array(ref1_0, kk, i) *= p_obs_cols(i, kk) / p_est_cols(i, kk);
        }
      }
    }
    
    // Reescalado filas de referencia
    for (int jj : parties1) {
      for (int i = 0; i < I; ++i) {
        if (!std::isnan(p_est_rows(i, jj)) && p_est_rows(i, jj) != 0.0) {
  ...
  // ---- IPF final por slice --------------------------------------------------
  for (int i = 0; i < I; ++i) {
    arma::mat slice_mat = pjk_array.slice(i);
    for (auto& v : slice_mat) {
      if (std::isnan(v) || std::isinf(v) || v < 0.0) v = 0.0;
    }
    double sum_yi = arma::sum(ym.row(i));
    slice_mat *= sum_yi;
